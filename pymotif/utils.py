...
def read_peaks_file(file_path):
    ...
    # Check if MACS peaks file
    if first_line.startswith('# This file is generated by MACS version'):
        return convert_to_homer(file_path)
    else:
        header_lines = count_header_lines(file_path) 
        return pd.read_csv(file_path, sep="\t", skiprows=header_lines)

def convert_to_homer(file_path):
    data = pd.read_csv(file_path, comment="#", delimiter="\t")
    data.columns = ["chr", "start", "end", "length", "abs_summit", "pileup", "-log10(pvalue)", "fold_enrichment", "-log10(qvalue)", "name"]

    data['chr'] = data['chr'].str.replace('chr', '')  # remove 'chr'
    # data['chr'] = data['chr'].apply(roman_to_arabic)  # convert Roman numerals to Arabic
    
    return data[['chr', 'start', 'end']]
